...
namespace LeapGestureRecognition
{
	public class SGRecorder
	{
		...
		public SGRecorder(EditStaticGestureViewModel editStaticGestureVM, MainViewModel mvm)
		{
		...
		private void recordSnapshotTimer_Tick(object sender, EventArgs e)
		{
			...
			{
				_editStaticGestureVM.AddInstance(new SGInstanceWrapper(_mvm.CurrentFrame));
			}
			else
			{
				_recordSnapshotTimer.Stop();
